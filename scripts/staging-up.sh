...
if [[ ! -f "${ENV_FILE}" ]]; then
  ...
  echo "Staging login password: ${SEED_PASS}"
fi
...
echo "Waiting for http://127.0.0.1:8088/health ..."
for i in $(seq 1 90); do
  if curl -sf "http://127.0.0.1:8088/health" >/dev/null 2>&1; then
    echo ""
    echo "Staging is up:"
    echo "  App (no cert warning):  http://127.0.0.1:8088"
    echo "  App (HTTPS/self-signed): https://127.0.0.1:8443"
    echo "  Health:   http://127.0.0.1:8088/health"
    echo "  Ready:    http://127.0.0.1:8088/ready"
